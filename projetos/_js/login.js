...
document.getElementById("btn-entrar").addEventListener("click", (e) => {
  var apiUrl = "https://andersonpadovani.com.br/crudSql/api.php";
  e.preventDefault();
  ...
  if (passw.validity.valid && email.validity.valid) {
    ...
    form_data.append(
      ...
      JSON.stringify({
        ...
        user_pass: CryptoJS.SHA1(passw.value).toString(),
      })
    );
    ...
    $.ajax({
      ...
      success: (resp) => {
        ...
        if (data.status === 200 && data.infoData != undefined) {
          ...
          modalSpan.innerHTML =
            "Sejá bem vindo, você será redirecionado automaticamente em 3s aguarde...";
          ...
          modalSpan.innerHTML =
            "Não conseguimos localizar o usuario no banco de dados!";
          logged.style.display = "flex";
        }
      },
    });
...
function lembrarUsuario() {
  ...
  if (
    ...
    localStorage.getItem(userPassw) != null
  ) {
